...
function allFlashing(octopuses) {
  const any = (element) => element % 10 === 0;
  return octopuses.map(i => i.every(any)).every(i => i);
}
...
for(let time = 1; time <= TIME; time++) {
  ...
  if(allFlashing(octopuses)) {
    flashingSteps.push(time);
  };
}
